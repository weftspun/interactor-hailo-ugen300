"""Does a HAR round-trip carry modified params into inference? load_params alone does not.

The control below proved load_params inert: halving the int16 kernels left the output
bit-identical, so the runner is serving a model built at optimize() time. This tries
save_har -> edit -> load_har, which has to rebuild, and keeps the same control.
...
LOG = open("/work/qzo_har.txt", "w", buffering=1)
...
for factor, label in ((1.5, "scales x1.5"), (0.5, "scales x0.5"), (10.0, "scales x10")):
    ...
    runner.save_har("/work/m.har")
    runner2 = ClientRunner(har="/work/m.har")
    with runner2.infer_context(InferenceContext.SDK_QUANTIZED) as ctx:
        o = np.asarray(runner2.infer(ctx, X)).reshape(B, -1)
